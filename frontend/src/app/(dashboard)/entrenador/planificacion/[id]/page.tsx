...
export default async function EntrenadorMacrocicloDetallePage({ params }: MacrocicloDetallePageProps) {
        ...
        <CardContent>
          {mesociclos.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No hay mesociclos en este macrociclo</p>
            </div>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {mesociclos.map((mesociclo) => {
                ...
                return (
                  <Link
                    key={mesociclo.id}
                    href={ENTRENADOR_ROUTES.planificacion.mesociclos.detalle(mesociclo.id)}
                    className="block"
                  >

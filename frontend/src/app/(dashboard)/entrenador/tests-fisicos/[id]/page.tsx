...
import { fetchTestFisicoEntrenador } from '@/features/entrenador/actions/fetch-tests-fisicos';
...
export default async function TestFisicoDetallePage({ params }: PageProps) {
  const { id } = await params;
  const test = await fetchTestFisicoEntrenador(id);
            ...
            <div>
              ...
              {test.atleta && (
                <Button variant="link" className="p-0 h-auto" asChild>
                  <Link href={`/entrenador/mis-atletas/${test.atleta.id}`}>
          ...
          <CardContent className="space-y-4">
            ...
            {test.test1500m && (
              <div>
                ...
                <p className="font-medium">{test.test1500m}</p>
              </div>
            )}
          </CardContent>
        </Card>
          ...
          <div className="flex flex-wrap gap-3">
            {test.atleta && (
              <>
                ...
                <Button variant="outline" asChild>
                  <Link href={`/entrenador/tests-fisicos?atletaId=${test.atleta.id}`}>
